...
<form className="selectionForm" onSubmit={onSearch}>
    <div>
        <input className="myInput" value={value} onChange={handleChange} placeholder="Search for locatio..."
        required/>
        <button className="mySearchButton" type="submit">&gt;</button>
    </div>
    <div>
        {locations.length > 0 && <select  size={locations.length+1} onChange={handleCity} value={selectedValue} ref={selectRef} placeholder="Select Location">
